...
PROJECT_OPTIONS = [
    server.ProjectOption(
        ...
        type="str",
        help=(
            "If given, during generate_project, uncompress the tarball at this path into the"
            " project dir."
        ),
    ),
    server.ProjectOption(
...
class Handler(server.ProjectAPIHandler):
    ...
    def server_info_query(self, tvm_version):
        return server.ServerInfo(
            ...
            model_library_format_path=(
                "" if IS_TEMPLATE else (API_SERVER_DIR / MODEL_LIBRARY_FORMAT_RELPATH)
            ),
    ...
    def _create_prj_conf(self, project_dir, options):
        ...
        with open(dest, mode) as f:
            ...
            if project_type == "host_driven":
                ...
                # deprecated stuff
                f.write("CONFIG_I2S_SUPPRESS_DEPRECATE_WARN=y\n")
                f.write("CONFIG_ADC_SUPPRESS_DEPRECATE_WARN=y\n")
                f.write("CONFIG_DAC_SUPPRESS_DEPRECATE_WARN=y\n")
                f.write("CONFIG_I2S_ENABLE_DEBUG_LOG=y\n")
                f.write("CONFIG_ESP_TASK_WDT_EN=n\n")
    ...
    def generate_project(self, model_library_format_path, standalone_crt_dir, project_dir, options):
        ...
        if version < ESPIDF_VERSION:
            message = (
                f"ESP-IDF version found is not supported: found {version}, expected"
                f" >={ESPIDF_VERSION}."
            )
